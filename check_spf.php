function check_spf_valid_or_not($domain_name)
{
    ...
    // echo $spf_ip_include_valid;die;
    $valid = 0;
    if(strpos($spf_ip_include_valid, '0') || @preg_match("#^0(.*)$#i", $spf_ip_include_valid))
    {
        $valid = 0;
    }
    elseif ( (!strpos($spf_ip_include_valid, '0') || !@preg_match("#^0(.*)$#i", $spf_ip_include_valid)) && (strpos($spf_ip_include_valid, '1') || @preg_match("#^1(.*)$#i", $spf_ip_include_valid) ) )
    {
        $valid=1;
    }
    return $valid;
